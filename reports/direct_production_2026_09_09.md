...
cell values and row data. Current catalog: 478 retained files, 95 clean CSV
tables, 40,477 rows. Counts include source-vintage overlap and diagnostics,
...
70 project tests pass using `.venv/bin/pytest -q`. A test initially compared a
...
30 figures have not started. Source-only packages and legacy partials are not
finished reconstructions. All Chapter 17 figures now have reconstruction or recovery
packages, but only lighting is verified. Literacy/basic education add two real comparison packages;
