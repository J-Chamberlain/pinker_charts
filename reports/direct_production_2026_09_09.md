...
cell values and row data. Current catalog: 369 retained files, 71 clean CSV
tables, 37,188 rows. Counts include source-vintage overlap and diagnostics,
...
Review gallery and PDF now contain 28 real figure packages / 56 pages. Pages
51-54 rendered from the final PDF and inspected for clipping and legibility.
...
60 project tests pass using `.venv/bin/pytest -q`. A test initially compared a
...
37 figures have not started. Source-only packages and legacy partials are not
finished reconstructions. Continue Figure 17-1 next, preserving source-family
