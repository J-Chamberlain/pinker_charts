<!-- canonical-state:start -->
# Figure 5-2: Child mortality, 1751-2013

Scientific status: `partial_match`. Execution: `blocked`.
Publication: `not_reviewed`. Artifact kind: `reconstruction`.

This generated summary and [figure.json](figure.json) supersede historical status claims below.
A historical verified classification is not a fresh publication review.

- Original reference: [plots/comparisons/kindle_reference_figure_5_2.png](plots/comparisons/kindle_reference_figure_5_2.png)
- Book period reconstruction: [plots/book_period/figure_5_2_book_period_reconstruction.png](plots/book_period/figure_5_2_book_period_reconstruction.png)
- Extended reconstruction: [plots/extended/figure_5_2_extended_reconstruction.png](plots/extended/figure_5_2_extended_reconstruction.png)
- Book period comparison: [plots/comparisons/figure_5_2_book_period_comparison.png](plots/comparisons/figure_5_2_book_period_comparison.png)
- Extended comparison: [plots/comparisons/figure_5_2_extended_comparison.png](plots/comparisons/figure_5_2_extended_comparison.png)
- Caption: [captions/caption.txt](captions/caption.txt)
- Provenance: [provenance/provenance.md](provenance/provenance.md)
- Anomaly review: [anomaly_reviews/anomaly_review.md](anomaly_reviews/anomaly_review.md)
- Review checklist: [review_checklist.md](review_checklist.md)
- Source log: [source_logs/source_log.md](source_logs/source_log.md)
- Search log: [search_iterations/search_iterations.md](search_iterations/search_iterations.md)
- Discrepancy log: [discrepancy_logs/discrepancy_log.md](discrepancy_logs/discrepancy_log.md)
- Legacy metadata: [metadata/metadata.json](metadata/metadata.json)
- Metadata: [figure.json](figure.json)

<!-- canonical-state:end -->

# Figure 5-2: Child mortality, 1751-2013

Status: `partial_match`

This package is a documented successor-proxy approximation, not a recovered
reproduction. The exact Roser (2016a) chart export assembled from UN child
mortality estimates and the Human Mortality Database was not found. An
immutable OWID repository copy of the contemporaneous CME Info (2016) input
was recovered, but it contains only 1970–2015 and cannot reproduce the full
chart. See `provenance/provenance.md` and `source_logs/source_log.md`.

## Canonical Artifacts

- Original reference: `plots/comparisons/kindle_reference_figure_5_2.png`
- Book-period reconstruction: `plots/book_period/figure_5_2_book_period_reconstruction.png`
- Extended reconstruction: `plots/extended/figure_5_2_extended_reconstruction.png`
- Book-period comparison: `plots/comparisons/figure_5_2_book_period_comparison.png`
- Extended comparison: `plots/comparisons/figure_5_2_extended_comparison.png`
- Caption: `captions/caption.txt`
- Provenance: `provenance/provenance.md`
- Anomaly review: `anomaly_reviews/anomaly_review.md`
- Metadata: `metadata/metadata.json`
- Reproduction script: `../../scripts/reconstruct_figure_5_2.py`
