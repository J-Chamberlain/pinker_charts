<!-- canonical-state:start -->
# Figure 9-4: Social spending, OECD countries, 1880-2016

Scientific status: `updated_equivalent`. Execution: `processed`.
Publication: `not_reviewed`. Artifact kind: `reconstruction`.

This generated summary and [figure.json](figure.json) supersede historical status claims below.
A historical verified classification is not a fresh publication review.

- Original reference: [plots/comparisons/kindle_reference_figure_9_4.png](plots/comparisons/kindle_reference_figure_9_4.png)
- Book period reconstruction: [plots/book_period/figure_9_4_book_period_reconstruction.png](plots/book_period/figure_9_4_book_period_reconstruction.png)
- Extended reconstruction: [plots/extended/figure_9_4_extended_reconstruction.png](plots/extended/figure_9_4_extended_reconstruction.png)
- Book period comparison: [plots/comparisons/figure_9_4_book_period_comparison.png](plots/comparisons/figure_9_4_book_period_comparison.png)
- Extended comparison: [plots/comparisons/figure_9_4_extended_comparison.png](plots/comparisons/figure_9_4_extended_comparison.png)
- Caption: [captions/caption.txt](captions/caption.txt)
- Provenance: [provenance/provenance.md](provenance/provenance.md)
- Anomaly review: [anomaly_reviews/anomaly_review.md](anomaly_reviews/anomaly_review.md)
- Review checklist: [review_checklist.md](review_checklist.md)
- Source log: [source_logs/source_log.md](source_logs/source_log.md)
- Search log: [search_iterations/search_iterations.md](search_iterations/search_iterations.md)
- Discrepancy log: [discrepancy_logs/discrepancy_log.md](discrepancy_logs/discrepancy_log.md)
- Legacy metadata: [metadata/metadata.json](metadata/metadata.json)
- Metadata: [figure.json](figure.json)

<!-- canonical-state:end -->

# Figure 9-4: Social spending, OECD countries, 1880-2016

Status: `updated_equivalent`

This directory was created for Track C on 2026-06-30. See `provenance/provenance.md`,
`source_logs/source_log.md`, and `anomaly_reviews/anomaly_review.md` for the source
recovery result and visual review.
